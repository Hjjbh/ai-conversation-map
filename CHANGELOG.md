...
## 2026-09-11｜阶段：D0-10A 定位熔断归因方案准备

### 修改目的

在不重试真实页面动作的前提下，对 D0-09B R01 的固定失败形状进行代码路径归因，并为下一版非暴露诊断和分级重新授权建立安全边界。

### 实际变更

1. 新增 `docs/stage-0/d0-10a-locate-failure-attribution-plan.md`，明确 `page=unknown + summary 零值 + CONVERSATION_CHANGED` 对应已有 session 在新一次 `run()` 入口前失效的动作前短路路径。
2. 区分可证明与不可证明结论：确认未进入滚动/高亮且已自动销毁，但不推断具体触发源、语义会话变化或操作人重复调用。
3. 定义候选 `changeAttribution` 固定白名单对象、枚举、first-wins 规则、禁止输出字段和合成测试门禁。
4. 将后续真实页面验证拆为独立的 D0-10B 被动哨兵基线和 D0-10C 单次定位动作；两者均需另行绑定、复核与 PO 批准，D0-09B 不得复用。
5. 更新阶段 README 与 T0-03 任务卡，登记 D0-10A 仅为待批准文档方案，当前没有新实现或页面运行额度。

涉及文件：`docs/stage-0/d0-10a-locate-failure-attribution-plan.md`、`docs/stage-0/README.md`、`docs/stage-0/task-cards.md`、`CHANGELOG.md`。

### 验证结果

- GDE 与 QSR 已分别完成只读归因和安全方案评审，结论一致：失败发生在动作前入口保护，现有 schema 无法区分首因，也不能证明操作人重复调用。
- 本轮仅修改文档；未修改探针代码，未访问 ChatGPT，未执行滚动、高亮、导航、输入、发送或其他页面动作。
- Stage 0 全套合成测试 `31/31 pass`；Markdown 本地链接检查为 33 个文件、0 个缺失；`git diff --check` 通过。

### 遗留事项

- D0-10A 尚待 PO 明确批准；批准前不得实现候选归因探针。
- D0-10B 与 D0-10C 尚未创建，当前没有任何真实页面运行额度。
- 稳定身份、跨刷新 locator、完整双向地图定位和真实分支仍未获得结论。
